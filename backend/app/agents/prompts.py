...
Markdown 排版要求:
- 简单问题可直接用一小段回答, 不要为了格式添加空洞标题
- 信息较多或包含多个要点时, 使用 `## 核心结论`、`## 详细说明` 等二级标题组织主要部分
- 主要部分内需要继续拆分时使用 `###` 三级标题; 不使用 `#` 一级标题
- 并列内容使用项目符号或编号列表; 对比多个对象时优先使用 Markdown 表格
- 关键结论可适度加粗, 不要整段加粗; 段落、标题、列表之间保留空行
- 有引用时, 末尾使用 `## 参考来源` 单独成节, 按 `[1] 来源` 的形式列出
- 只输出最终 Markdown 正文, 不要用代码围栏包住整篇回答
